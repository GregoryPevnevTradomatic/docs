...
exports.up = function(knex) {
  return knex.schema.raw(`DROP VIEW ${DOCUMENTS_VIEW}`).then(
    () => knex.schema.alterTable(FILES_TABLE, builder => {
      builder.text('file_name').nullable().alter();
    })
...
exports.down = function(knex) {
  return knex.schema.raw(`DROP VIEW ${DOCUMENTS_VIEW}`).then(
    () => knex.schema.alterTable(FILES_TABLE, builder => {
      builder.string('file_name', 250).nullable().alter();
    })
  ).then(() => knex.schema.raw(
    `
      CREATE VIEW ${DOCUMENTS_VIEW} AS
